...
const queryDatabase = async () => {
  try {
    ...
    const allDepartmentNumbersAndNumberOfEmployees = await execQuery(
      `SELECT department_no, COUNT(employee_no) AS "NumberOfEmployees" FROM departments 
      LEFT JOIN employees ON employees.department_no = departments.dept_no GROUP BY  department_no;`
    );
    // Q-2 => Sum of the salaries of all employees.
    const sumOfSalaries = await execQuery(`SELECT SUM(salary) FROM employees;`);
    ...
    const sumOfSalariesOfEmployeesPerDepartment = await execQuery(
      `SELECT employees.department_no, departments.title, SUM(salary) FROM employees 
      JOIN departments ON employees.department_no = departments.dept_no 
...
app.get('/allDepartmentNumbersAndNumberOfEmployees', (req, res) => {
  let sql = `SELECT employees.department_no, COUNT(employee_no) AS "NumOfEmployees" FROM employees 
...
connect()
  .then(() => {
    ...
    const allDepartmentNumbersAndNumberOfEmployees = execQuery(
      `SELECT employees.department_no, COUNT(employee_no) AS "NumOfEmployees" FROM employees 
      RIGHT JOIN departments ON employees.department_no = departments.dept_no GROUP BY  department_no;`,
    );
